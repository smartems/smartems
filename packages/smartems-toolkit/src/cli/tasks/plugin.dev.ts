...
const yarnlink = useSpinner<void>('Linking local toolkit', async () => {
  try {
    ...
    await execa('yarn', ['remove', '@smartems/toolkit']);
  } catch (e) {
    console.log('\n', e.message, '\n');
  }
  await execa('yarn', ['link', '@smartems/toolkit']);
  ...
  console.log('Added dependencies required by local @smartems/toolkit.  Do not checkin this package.json!');
