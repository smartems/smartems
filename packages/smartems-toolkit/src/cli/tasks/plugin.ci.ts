 ...
 *  3. prepare smartems environment in: `~/ci/smartems-test-env`
 */
const packagePluginRunner: TaskRunner<PluginCIOptions> = async () => {
  ...
  const smartemsEnvDir = path.resolve(ciDir, 'smartems-test-env');
  await execa('rimraf', [packagesDir, distDir, smartemsEnvDir]);
  ...
  let p = path.resolve(smartemsEnvDir, 'plugins', pluginInfo.id);
  ...
  p = path.resolve(smartemsEnvDir, 'custom.ini');
  const customIniBody =
    `# Autogenerated by @smartems/toolkit \n` +
    `[paths] \n` +
    `plugins = ${path.resolve(smartemsEnvDir, 'plugins')}\n` +
    `\n`; // empty line
 ...
 * 3. Test (end-to-end)
 *
 *  deploy the zip to a running smartems instance
 *
 */
const testPluginRunner: TaskRunner<PluginCIOptions> = async ({ full }) => {
  ...
  try {
    ...
    results.smartems = frontendSettings.data.buildInfo;

    console.log('Grafana: ' + JSON.stringify(results.smartems, null, 2));
    ...
    await execa('cp', [
      'node_modules/@smartems/toolkit/src/plugins/e2e/commonPluginTests.ts',
...
const pluginReportRunner: TaskRunner<PluginCIOptions> = async ({ upload }) => {
  ...
  const report: PluginBuildReport = {
    ...
    smartemsVersion: getGrafanaVersions(),
  };
  ...
  const SMARTEMS_API_KEY = process.env.SMARTEMS_API_KEY;
  if (!SMARTEMS_API_KEY) {
  ...
  const info = await axios.post(url, report, {
    headers: { Authorization: 'Bearer ' + SMARTEMS_API_KEY },
  });
  if (info.status === 200) {
