...
The LDAP integration in smartEMS allows your smartEMS users to login with their LDAP credentials. You can also specify mappings between LDAP
group memberships and smartEMS Organization user roles.

## Supported LDAP Servers

smartEMS uses a [third-party LDAP library](https://github.com/go-ldap/ldap) under the hood that supports basic LDAP v3 functionality.
...
## smartEMS LDAP Configuration

Depending on which LDAP server you're using and how that's configured your smartEMS LDAP configuration may vary.
...
smartEMS has an LDAP debug view built-in which allows you to test your LDAP configuration directly within smartEMS. At the moment of writing, only smartEMS admins can use the LDAP debug view.
...
In `[[servers.group_mappings]]` you can map an LDAP group to a smartEMS organization and role.  These will be synced every time the user logs in, with LDAP being
the authoritative source. So, if you change a user's role in the smartEMS Org. Users page, this change will be reset the next time the user logs in. If you
...
smartEMS does support receiving information from multiple LDAP servers.
