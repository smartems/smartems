...
You can configure many different oauth2 authentication services with smartEMS using the generic oauth2 feature. Below you
can find examples using Okta, BitBucket, OneLogin and Azure.

This callback URL must match the full HTTP address that you use in your browser to access smartEMS, but with the prefix path of `/login/generic_oauth`.
...
smartEMS will attempt to determine the user's e-mail address by querying the OAuth provider as described below in the following order until an e-mail address is found:
...
**Note**: Only available in smartEMS v6.4+.
3. Check for the presence of an e-mail address in the `attributes` map encoded in the OAuth `id_token` parameter. By default smartEMS will perform a lookup into the attributes map using the `email:primary` key, however, this is configurable and can be adjusted by using the `email_attribute_name` configuration option.
...
2.  Add an App to the smartEMS Connector:
    - Display Name: smartEMS

    then:
3.  Under the SSO tab on the smartEMS App details page you'll find the Client ID and Client Secret.

    Your OneLogin Domain will match the url you use to access OneLogin.

    Configure smartEMS as follows:
...
> Note: It's important to ensure that the [root_url](/installation/configuration/#root-url) in smartEMS is set in your Azure Application Reply URLs (App -> Settings -> Reply URLs)
...
> Only available in smartEMS v6.0 and above.

Some OAuth2 providers might not support `client_id` and `client_secret` passed via Basic Authentication HTTP header, which
results in `invalid_client` error. To allow smartEMS to authenticate via these type of providers, the client identifiers must be
...
In the following example user will get `Editor` as role when authenticating. The value of the property `role` will be the resulting role if the role is a proper smartEMS role, i.e. `Viewer`, `Editor` or `Admin`.
