+++
title = "What's New in smartEMS v5.2"
description = "Feature and improvement highlights for smartEMS v5.2"
keywords = ["smartems", "new", "documentation", "5.2"]
type = "docs"
[menu.docs]
name = "Version 5.2"
identifier = "v5.2"
...
# What's New in smartEMS v5.2

smartEMS v5.2 brings new features, many enhancements and bug fixes. This article will detail the major new features and enhancements.

- [Elasticsearch alerting]({{< relref "#elasticsearch-alerting" >}}) it's finally here!
- [Native builds for ARM]({{< relref "#native-builds-for-arm" >}}) native builds of smartEMS for many more platforms!
- [Improved Docker image]({{< relref "#improved-docker-image" >}}) with support for docker secrets
- [Security]({{< relref "#security" >}}) make your smartEMS instance more secure
...
smartEMS v5.2 ships with an updated Elasticsearch data source with support for alerting. Alerting support for Elasticsearch has been one of
...
smartEMS v5.2 brings an improved build pipeline with cross-platform support. This enables native builds of smartEMS for ARMv7 (x32) and ARM64 (x64).
...
The smartEMS docker image adds support for Docker secrets which enables you to supply smartEMS with configuration through files. More
...
Starting from smartEMS v5.2, when you login with the administrator account using the default password you'll be presented with a form to change the password.
We hope this encourages users to follow smartEMS's best practices and change the default administrator password.
...
By popular demand smartEMS now includes support for an alert notification channel type for [Discord](https://discordapp.com/).
...
Starting from smartEMS v5.2, a modified time range or variable are no longer saved by default. To save a modified
...
smartEMS v5.2 adds support for specifying an existing folder or creating a new one when importing a dashboard - a long-awaited feature since
smartEMS v5.0 introduced support for dashboard folders and permissions. The import dashboard page has also got some general improvements
