+++
title = "What's New in smartEMS v4.0"
description = "Feature and improvement highlights for smartEMS v4.0"
keywords = ["smartems", "new", "documentation", "4.0"]
type = "docs"
[menu.docs]
name = "Version 4.0"
identifier = "v4.0"
...
If you have configured an external image store in the smartems.ini config file (s3, webdav, and azure_blob options available)
you can get very rich notifications with an image of the graph and the metric
values all included in the notification.

### Annotations

Alert state changes are recorded in a new annotation store that is built into smartEMS. This store
currently only supports storing annotations in smartEMS's own internal database (mysql, postgres or sqlite).
The smartEMS annotation storage is currently only used for alert state changes but we hope to add the ability for users
to add graph comments in the form of annotations directly from within smartEMS in a future release.
...
<div class="row">
  <div class="medium-6 columns">
    <p>
      smartEMS is so often used on wall mounted TVs that we figured a clean TV mode would be
...
to another you will insert the panel and smartEMS will automatically make room for it.
...
smartEMS v4 introduces a number of really powerful keyboard shortcuts. You can now focus a panel
...
There are no breaking changes. Old dashboards and features should work the same. smartEMS-server will automatically upgrade its db
schema on restart. It's advisable to do a backup of smartEMS's database before updating.
