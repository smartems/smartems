...
Please read [Using Elasticsearch in smartEMS](/features/datasources/elasticsearch/#querying-logs-beta) for more detailed information on how to get started and use it.
...
Please read [Using InfluxDB in smartEMS](/features/datasources/influxdb/#querying-logs-beta) for more detailed information on how to get started and use it.
...
For example, lets say a user is removed from an LDAP group. In previous versions of smartEMS an admin would have to
wait for the user to logout or the session to expire for the smartEMS permissions to update, a process that can take days.

With active sync the user would be automatically removed from the corresponding team in smartEMS or even logged out and disabled if no longer
belonging to an LDAP group that gives them access to smartEMS.

[Read more](/auth/enhanced_ldap/#active-ldap-synchronization)

### SAML Authentication

Built-in support for SAML is now available in smartEMS Enterprise.

[See docs]({{< relref "auth/saml.md" >}})

### Team Sync for GitHub OAuth

When setting up OAuth with GitHub it's now possible to sync GitHub teams with Teams in smartEMS.
