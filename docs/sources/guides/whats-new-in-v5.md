...
# What's New in smartEMS v5.0

This is the most substantial update that smartEMS has ever seen. This article will detail the major new features and enhancements.
...
- [Permissions]({{< relref "#dashboard-folders" >}}) on folders and dashboards helps manage larger smartEMS installations.
...
Almost every page has seen significant UX improvements. All pages (except dashboard pages) have a new tab-based layout that improves navigation between pages. The side menu has also changed quite a bit. You can still hide the side menu completely if you click on the smartEMS logo.
...
The big new feature that comes with smartEMS v5.0 is dashboard folders. Now you can organize your dashboards in folders,
...
A team is a new concept in smartEMS v5. They are simply a group of users that can be used in the new permission system for dashboards and folders. Only an admin can create teams.
...
In previous versions of smartEMS, you could only use the API for provisioning data sources and dashboards.
...
Data sources can now be setup using config files. These data sources are by default not editable from the smartEMS GUI.
...
in sync with dashboards in smartEMS's database. The dashboard provisioner has multiple advantages over the old
`[dashboard.json]` feature. Instead of storing the dashboard in memory we now insert the dashboard into the database,
which makes it possible to star them, use one as the home dashboard, set permissions and other features in smartEMS that
