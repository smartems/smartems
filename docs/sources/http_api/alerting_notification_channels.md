...
The identifier (id) of a notification channel is an auto-incrementing numeric value and is only unique per smartEMS install.

The unique identifier (uid) of a notification channel can be used for uniquely identify a notification channel between
multiple smartEMS installs. It's automatically generated if not provided when creating a notification channel. The uid
...
smartEMS installs, see [alert notification channel provisioning](/administration/provisioning/#alert-notification-channels)
...
alert notification channels in smartEMS UI when configuring alert rule.
