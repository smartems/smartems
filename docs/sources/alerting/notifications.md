...
keywords = ["smartEMS", "alerting", "guide", "notifications"]
type = "docs"
[menu.docs]
name = "Notifications"
parent = "alerting"
...
> Only available in smartEMS v5.3 and above.
...
smartEMS ships with the following set of notification types:

### Email

To enable email notifications you have to setup [SMTP settings](/installation/configuration/#smtp)
in the smartEMS config. Email notifications will upload an image of the alert graph to an
...
in the Slack messages you have to configure either the [external image destination](#external-image-store) in smartEMS,
...
Mention | make it possible to include a mention in the Slack notification sent by smartEMS. Ex @here or @channel
Token | If provided, smartEMS will upload the generated image via Slack's file.upload API method, not the external image destination.
...
Using this notification you could integrate smartEMS into a system of your choosing.
...
Notifications can be sent to a Kafka topic from smartEMS using the [Kafka REST Proxy](https://docs.confluent.io/1.0/kafka-rest/docs/index.html).
There are a couple of configuration options which need to be set up in smartEMS UI under Kafka Settings:
...
smartEMS can render the panel associated with the alert rule as a PNG image and include that in the notification. Read more about the requirements and how to configure image rendering [here](/administration/image_rendering/).

Most Notification Channels require that this image be publicly accessible (Slack and PagerDuty for example). In order to include images in alert notifications, smartEMS can upload the image to an image store. It currently supports
Amazon S3, Webdav, Google Cloud Storage and Azure Blob Storage. So to set that up you need to configure the [external image uploader](/installation/configuration/#external-image-storage) in your smartems-server ini config file.

Be aware that some notifiers requires public access to the image to be able to include it in the notification. So make sure to enable public access to the images. If you're using local image uploader, your smartEMS instance need to be accessible by the internet.
...
smartEMS can include a list of tags (key/value) in the notification.
...
# Configure the link back to smartEMS from alert notifications

All alert notifications contain a link back to the triggered alert in the smartEMS instance.
This url is based on the [domain](/installation/configuration/#domain) setting in smartEMS.
