...
smartEMS will detect this and automatically refresh a variable when one of it's containing variables change.
...
is valid in the given context where the variable is used. smartEMS tries to solve this by allowing each data source plugin to
inform the templating interpolation engine what format to use for multiple values.

Note that the *Custom all value* option on the variable will have to be left blank for smartEMS to format all values into a single string.
...
If you do not want smartEMS to do this automatic regex escaping and formatting your only option is to disable the *Multi-value* or *Include All option*
options.
...
smartEMS has global built-in variables that can be used in expressions in the query editor.

### Time range variables

smartEMS has two built in time range variables in `$__from` and `$__to`. They are currently always interpolated
as epoch milliseconds. These variables are only available in smartEMS v6.0 and above.
...
This is used in the WHERE clause for the InfluxDB data source. smartEMS adds it automatically to InfluxDB queries when in Query Editor Mode. It has to be added manually in Text Editor Mode: `WHERE $timeFilter`.
...
smartEMS to dynamically create new panels or rows based on what values you have selected you can use the *Repeat* feature.

If you have a variable with `Multi-value` or `Include all value` options enabled you can choose one panel and have smartEMS repeat that panel
...
By choosing `horizontal` the panels will be arranged side-by-side. smartEMS will automatically adjust the width
