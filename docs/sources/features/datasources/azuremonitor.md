...
# Using Azure Monitor in smartEMS

> Officially released in smartEMS v6.0.0

As of smartEMS 6.0, the Azure Monitor plugin has been moved into smartEMS so it now ships with built-in support for Azure Monitor.
...
1. Accessed from the smartEMS main menu, newly installed data sources can be added immediately within the Data Sources section. Next, click the "Add data source" button in the upper right. The Azure Monitor data source will be available for selection in the Cloud section in the list of data sources.

2. In the name field, smartEMS will automatically fill in a name for the data source - `Azure Monitor` or something like `Azure Monitor - 3`. If you are going to configure multiple data sources then change the name to something more informative.
...
smartEMS alerting is supported for the Azure Monitor service. This is not Azure Alerts support. Read more about how alerting in smartEMS works [here]({{< relref "alerting/rules.md" >}}).
...
smartEMS alerting is supported for Application Insights. This is not Azure Alerts support. Read more about how alerting in smartEMS works [here]({{< relref "alerting/rules.md" >}}).
...
To make writing queries easier there are several smartEMS macros that can be used in the where clause of a query:

- `$__timeFilter()` - Expands to
  `TimeGenerated ≥ datetime(2018-06-05T18:09:58.907Z) and`
  `TimeGenerated ≤ datetime(2018-06-05T20:09:58.907Z)` where the from and to datetimes are from the smartEMS time picker.

- `$__timeFilter(datetimeColumn)` - Expands to
  `datetimeColumn ≥ datetime(2018-06-05T18:09:58.907Z) and`
  `datetimeColumn ≤ datetime(2018-06-05T20:09:58.907Z)` where the from and to datetimes are from the smartEMS time picker.

- `$__timeFrom()` - Returns the From datetime from the smartEMS picker. Example: `datetime(2018-06-05T18:09:58.907Z)`.
