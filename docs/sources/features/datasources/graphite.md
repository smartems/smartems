...
# Using Graphite in smartEMS

smartEMS has an advanced Graphite query editor that lets you quickly navigate the metric space, add functions,
...
*Access* | Server (default) = URL needs to be accessible from the smartEMS backend/server, Browser = URL needs to be accessible from the browser.
...
> client side by smartEMS. And depending on your consolidation function only one or two can be correct at the same time.
...
Graphite 1.1 introduced tags and smartEMS added support for Graphite queries with tags in version 5.0. To create a variable using tag values, then you need to use the smartEMS functions `tags` and `tag_values`.
...
Multi-value variables in tag queries use the advanced formatting syntax introduced in smartEMS 5.0 for variables: `{var:regex}`. Non-tag queries will use the default glob formatting for multi-value variables.
