...
# Using Microsoft SQL Server in smartEMS

> Only available in smartEMS v5.1+.

smartEMS ships with a built-in Microsoft SQL Server (MSSQL) data source plugin that allows you to query and visualize data from any Microsoft SQL Server 2005 or newer, including Microsoft Azure SQL Database.
...
*Encrypt* | This option determines whether or to which extent a secure SSL TCP/IP connection will be negotiated with the server, default `false` (smartEMS v5.4+).
...
>
> From smartEMS 5.0.0, template variable values are only quoted when the template variable is a `multi-value`.
