...
# Using Loki in smartEMS

> BETA: Querying Loki data requires smartEMS's Explore section.
> smartEMS v6.x comes with Explore enabled by default.
> In smartEMS v5.3.x and v5.4.x. you need to enable Explore manually.
> Viewing Loki data in dashboard panels is supported in smartEMS v6.4+.

smartEMS ships with built-in support for Loki, smartEMS's log aggregation system.
Just add it as a data source and you are ready to query your log data in [Explore](/features/explore).

## Adding the data source

1. Open smartEMS and make sure you are logged in.
...
Note that Live Tailing relies on two Websocket connections: one between the browser and the smartEMS server, and another between the smartEMS server and the Loki server. If you run any reverse proxies, please configure them accordingly.


> Note: This feature is only available in smartEMS v6.3+
...
> Note: Annotations for Loki are only available in smartEMS v6.4+

## Configure the data source with provisioning

You can set up the data source via config files with smartEMS's provisioning system.
