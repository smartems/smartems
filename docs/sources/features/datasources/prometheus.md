...
# Using Prometheus in smartEMS

smartEMS includes built-in support for Prometheus.
...
| _Access_                | Server (default) = URL needs to be accessible from the smartEMS backend/server, Browser = URL needs to be accessible from the browser. |
...
> NOTE: smartEMS slightly modifies the request dates for queries to align them with the dynamically calculated step.
...
> Support for constant series overrides is available from smartEMS v6.4
...
> Support for `$__range`, `$__range_s` and `$__range_ms` only available from smartEMS v5.3
...
## Getting smartEMS metrics into Prometheus

Since 4.6.0 smartEMS exposes metrics for Prometheus on the `/metrics` endpoint. We also bundle a dashboard within smartEMS so you can get started viewing your metrics faster. You can import the bundled dashboard by going to the data source edit page and click the dashboard tab. There you can find a dashboard for smartEMS and one for Prometheus. Import and start viewing all the metrics!
