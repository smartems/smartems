...
# Using Google Stackdriver in smartEMS

> Available as a beta feature in smartEMS v5.3.x and v5.4.x.
> Officially released in smartEMS v6.0.0

smartEMS ships with built-in support for Google Stackdriver. Just add it as a data source and you are ready to build dashboards for your Stackdriver metrics.
...
There are two ways to authenticate the Stackdriver plugin - either by uploading a Google JWT file, or by automatically retrieving credentials from Google metadata server. The latter option is only available when running smartEMS on GCE virtual machine.

### Using a Google Service Account Key File

To authenticate with the Stackdriver API, you need to create a Google Cloud Platform (GCP) Service Account for the Project you want to show data for. A smartEMS data source integrates with one GCP Project. If you want to visualize data from multiple GCP Projects then you need to create one data source per GCP Project.
...
If smartEMS is running on a Google Compute Engine (GCE) virtual machine, it is possible for smartEMS to automatically retrieve default credentials from the metadata server. This has the advantage of not needing to generate a private key file for the service account and also not having to upload the file to smartEMS. However for this to work, there are a few preconditions that need to be met.
...
Stackdriver metrics can be of different kinds (GAUGE, DELTA, CUMULATIVE) and these kinds have support for different aggregation options (reducers and aligners). The smartEMS query editor shows the list of available aggregation methods for a selected metric and sets a default reducer and aligner when you select the metric. Units for the Y-axis are also automatically selected by the query editor.
...
The `Alignment Period` groups a metric by time if an aggregation is chosen. The default is to use the GCP Stackdriver default groupings (which allows you to compare graphs in smartEMS with graphs in the Stackdriver UI).
...
| *Alignment periods* | Returns a list of all alignment periods that are available in Stackdriver query editor in smartEMS |
