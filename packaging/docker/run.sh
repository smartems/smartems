...
if [ $PERMISSIONS_OK -eq 1 ]; then
    echo "You may have issues with file permissions, more information here: http://docs.smartems.org/installation/docker/#migration-from-a-previous-version-of-the-docker-container-to-5-1-or-later"
fi
...
if [ ! -z "${GF_INSTALL_PLUGINS}" ]; then
  ...
  for plugin in ${GF_INSTALL_PLUGINS}; do
    ...
    if [[ $plugin =~ .*\;.* ]]; then
        ...
        smartems-cli --pluginUrl "${pluginUrl}" --pluginsDir "${GF_PATHS_PLUGINS}" plugins install ${pluginWithoutUrl}
    else
