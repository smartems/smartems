...
build-docker-dev: ## Build Docker image for development (fast).
	...
	cp dist/smartems-latest.linux-x64.tar.gz packaging/docker
	cd packaging/docker && docker build --tag smartems/smartems:dev .

build-docker-full: ## Build Docker image for development.
	@echo "build docker container"
	docker build --tag smartems/smartems:dev .
